...
else
{
    ...
    $tpl->assign_block_vars('filter_row', array(
        ...
        'SELECTED' => ( $id == strtolower("None") ) ? ' selected="selected"' : '',
    ...
    $tpl->assign_block_vars('filter_row', array(
        'VALUE'    => strtolower("None"),
        'SELECTED' => ( $id == strtolower("NULL") ) ? ' selected="selected"' : '',
	...
	$result = $db->query($sql);
        ...
        while ( $row = $db->fetch_record($result) )
        {
          $tpl->assign_block_vars('filter_row', array(
              ...
              'SELECTED' => ( $id == strtolower($row['class_armor_type']) ) ? ' selected="selected"' : '',
	...
	// Moved the class/race/faction information to the database
        ...
        while ( $row = $db->fetch_record($result) )
        {
            $tpl->assign_block_vars('filter_row', array(
                ...
                'SELECTED' => ( $temp_filter == $row['class_name']) ? ' selected="selected"' : '',
