...
/**
 * This class handles add default values add & update
 * @subpackage ManageCTRT
 */
class CTRT_AddDefaultValue extends CTRT_ManageDefaultValue
{
    var $data = array();           // Holds own_raid data if URI_ID is set         @var data

    function CTRT_AddDefaultValue()
    {
        parent::eqdkp_admin();
        ...
        if ( isset($_POST['delete']) )
        {
            if ( isset($_POST['compare_ids']) )
            {
                foreach ( $_POST['compare_ids'] as $keyID )
                {
                    ...
                    $items_ids[] = $item[0]['dvalue_id'];
                    $items_names[]  = $item[0]['item_name'];
                }
    ...
    function process_add()
    {
        ...
        if (!$this->daoAddItem->insert(array('item_wowid' => $add_item['wowid'],
                                             'item_name' => $add_item['name'],
    ...
    function display_form()
    {
        ...
        $this->_eqdkp->set_vars(array(
            ...
            'template_file' => 'admin/adddefaultvalue.html',
            'display'       => true)
        );
    }
}
