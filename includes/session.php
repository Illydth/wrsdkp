...
class User extends Session
{
    ...
    */
	...
	/**
    * Attempt to log in a user through e107: IN PROGRESS
    *
    * @param $username
    * @param $password
    * @param $auto_login Save login in cookie?
    * @return bool
    */
    function login_e107($username, $password, $auto_login)
    {
        global $user, $db, $eqdkp;

        // Installed in our CMS Database, simply select from a different table.
        
        // Table Prefix for CMS tables.
		$table_prefix = "e107_TC_";        
		...
		// Table Name for the User's Table.
		...
		$sql = sprintf(	"SELECT ".$db_user_id.",". $db_user_name .",".$db_user_email.",".$db_user_password.
						...
						" WHERE ".$db_user_name." = '%s'", addslashes($username)
				);
        ...
        if ( $row = $db->fetch_record($result) )
        {
            ...
            if (md5($password) == $row[$db_user_password])
            {
				$sql = 'SELECT user_id
		                FROM ' . USERS_TABLE . "
		                WHERE username='" . addslashes($username) . "'";
            	...
            	// Check if the user doesn't already have an account.
            	if (!$db->num_rows($result))
            	{
            		$db->free_result($result);
            		
	            	// At this point the user should be logged in, lets create a user entry for
	            	// this user in the eqdkp tables.
					$query = $db->build_query('INSERT', array(
						'user_id' => $row[$db_user_id],
						'username'    => $row[$db_user_name],
		            ...
		            $db->query('INSERT INTO ' . USERS_TABLE . $query);
            	...
            	// Ok, user created, move on with the rest of the EQDKP Login procedure.
                $auto_login = ( !empty($auto_login) ) ? md5($password) : '';

                return $this->create($row[$db_user_id], $auto_login, true);
            }
        }
        return false;
    }
